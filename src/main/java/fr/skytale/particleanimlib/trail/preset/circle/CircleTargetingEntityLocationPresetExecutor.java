...
public class CircleTargetingEntityLocationPresetExecutor extends ATrailPresetExecutor {
    @Override
    public void applyPreset(TrailBuilder trailBuilder, JavaPlugin plugin) {
        ...
        circleBuilder.setPosition(new TargetingLocationTrailPosition(
                true,
                true,
                1f,
                new Vector(0, 0.9, 0),
                (animationToShowCloned, originLocation, player, trailPlayerData) -> {
                    ...
                    Entity closestEntity = closestEntityUUID == null ? null : nearbyEntities.stream()
                            .filter(entity -> entity.getUniqueId().equals(closestEntityUUID))
                            .findAny()
                            .orElse(null);

                    //get closest entity current location (the projectile will aim where the entity was and not follow it)
                    return closestEntity != null ? closestEntity.getLocation().clone() : null;
                }
        ));
